...
def player_won_battle(player_monature, enemy_monature, battle_type) -> str:
    ...
    if pmon[options[battle_type]] > emon[options[battle_type]]: return PLAYER_WON
    elif pmon[options[battle_type]] < emon[options[battle_type]]: return PLAYER_LOST
    else: return TIE
...
Battle {battle}
-----
Your {currentp.name} {formatRes} the enemy's {currente.name} in a {mode}.""")
            battle += 1
        round += 1
